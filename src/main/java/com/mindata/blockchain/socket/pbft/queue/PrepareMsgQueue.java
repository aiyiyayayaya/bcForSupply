...
public class PrepareMsgQueue extends AbstractVoteMsgQueue {
    ...
    protected void deal(VoteMsg voteMsg, List<VoteMsg> voteMsgs) {
        ...
        commitMsg.setType(VoteType.COMMIT);
        commitMsg.setSendId(AppId.value);
        //开始校验并决定是否进入commit阶段
        //校验该vote是否合法
        if (commitMsgQueue.hasOtherConfirm(hash, voteMsg.getSequence_no())) {
             agree(commitMsg, false);
        } else {
            //开始校验拜占庭数量，如果agree的超过2f + 1，就commit
            long agreeCount = voteMsgs.stream().filter(VoteMsg::isResult).count();
